...
void bigleaf3(vec3 p0, vec3 v)
{
	...
	for(int i = 0; i < 2000; i ++)
	{
		...
		for(int ii = 0; ii < wid; ii ++)
		{
			...
			vv*=quat(-PI * blend(0, 0.005, real(ii) / wid0, 4), v);
			//vv = vv + up * 0.001;
			vv.norm();
...
void meigui(vec3 p0, vec3 v0)
{
	...
	for(int i = 0; i < 1000; i ++)
	{
		...
		for(int ii = 0; ii < wid; ii ++)
		{
			...
			//vv.rot(-PI * blend(0, 0.05, aii, 8) * blend(0, 2, ai - 0.25, 2), v);
			//vv.rot(-PI * blend(0, 0.05, real(ii) / wid0, 8) * blend(0, 2, aii - 0.5, 2), v);
			vv*=quat(-PI * blend(0, meiguisize*0.02, real(ii) / wid0, 4), v);
			//vv = vv + up * 0.001;
			vv.norm();
...
{	
	vec3 vcp4 = v* quat(rrnd(-PI, PI), vec3(rrnd(-1, 1), rrnd(-1, 1), rrnd(-1, 1)).normcopy());
	vcp4.z = -1;
	vcp4.norm();
	vec3 cp1[] = {
		v,
		v* quat(rrnd(-PI, PI), vec3(rrnd(-1, 1), rrnd(-1, 1), rrnd(-1, 1)).normcopy()),
		v* quat(rrnd(-PI, PI), vec3(rrnd(-1, 1), rrnd(-1, 1), rrnd(-1, 1)).normcopy()),
		vcp4
	};	
	...
	for(int i = 0; i < len; i ++)
	{
		...
		for(int ii = 0; ii < 400; ii ++)
		{
			...
			norm*=  quat(ang1 < PI ? ang1 : ang1 - PI, v);
...
void curvedleaf(vec3 p0, vec3 v0)
{
	...
	for(int i = 0; i < 800; i ++)
	{
		...
		for(int ii = 0; ii < wid; ii ++)
		{
			...
			vv1*=quat(-PI * blend(0, cur2, real(ii) / wid0, 4), v);
			vv1.norm();
			vv2*=  quat(PI * blend(0, cur2, real(ii) / wid0, 4), v);
			vv2.norm();
...
void teng(vec3 p, vec3 v, int len, real wid0, int depth = 0)
{
	vec3 cp1[] = {
		v,
		v* quat(rrnd(-PI, PI) * 2.5, vec3(rrnd(-1, 1), rrnd(1, 1), rrnd(-1, 1)).normcopy()),
		v* quat(rrnd(-PI, PI) * 2.5, vec3(rrnd(-1, 1), rrnd(1, 1), rrnd(-1, 1)).normcopy()),
		v* quat(rrnd(-PI, PI) * 2.5, vec3(rrnd(-1, 1), rrnd(1, 1), rrnd(-1, 1)).normcopy())
	};	
	...
	for(int i = 0; i < len; i ++)
	{
		...
		if(i - last > step || i == len - 1)
		{
			...
			if(rrnd(0, 1000) < (depth == 0 ? 120 : 100) && wid > 0.0001)
			{
				...
				{
					teng(p, v * deta1, rrnd(0.1, 0.8) * len / 2, depth == 0 ? rrnd(0.08, 0.45) * wid : rrnd(0.5, 0.7) * wid, depth + 1);		
